...
import { BACKGROUND_COLOR, COMMUNITY_ROUT } from "../../utils/const";
...
const AuthPage: FC = () => {
    ...
    const handleAuth = async () => {
        if (location.pathname === '/login') { 
            ...
            await UStore.registration(email, password)
        }
        if (UStore.isAuth) {
            navigate(COMMUNITY_ROUT)
        }
    };
            ...
            <Container size='35rem' >
                <Card padding="2rem 1.5rem 2.5rem" radius="1.875rem">
                    <Flex gap="2rem" direction="column">
                        <ActionIcon variant="transparent" size="1.5rem">
                            <Image onClick={() => navigate(-1)} src={arrow} alt='Вернуться назад' height='1.5rem' width='1.5rem'/>
                        </ActionIcon>
                        <Card padding="0 2.5rem">
                        <Flex gap="3rem" direction="column">
                            <Group position="center" spacing="0.5rem">
                                <ThemeIcon radius="0.625rem" size="2rem" color="orange.5">
                                    <Image  height='1rem' width='1rem' src={logo}/>
                                </ThemeIcon>
